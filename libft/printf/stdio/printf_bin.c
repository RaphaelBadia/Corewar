...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2016/12/05 10:05:04 by rbadia            #+#    #+#             */
/*   Updated: 2016/12/05 10:15:24 by rbadia           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
ssize_t				printf_bin(va_list ap, void *datavoid)
{
	t_pf			*data;
	uintmax_t		nb;
	...
	&& data->precision == -1)
	{
		...
		data->precision = max_((data->precision == -1 ? 0 : data->precision),
			nbrbase_len(nb, "01") + 1);
	return (put_uint(nb, data, "01", NULL));
}
